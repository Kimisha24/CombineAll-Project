const connection = require('../combinedb');


function route(app) {

    app.get('/jobwithcrud', function (req, res) {
        try {
            res.render('job-with-crud/jobform');
        } catch (err) {
            console.log(err);
        }
    ...
    app.get('/form2', function (req, res) {
        try {
            ...
            res.render('job-with-crud/form2', { msg });
        } catch (err) {
            console.log(err);
        }
    });
    ...
    app.post('/formsubmit', async (req, res) => {
        ...
        try {
            ...
            var result = await connection.promise().query(sql);
            ...
            var details = await connection.promise().query(sqldetail, [first_name, last_name, designation, current_add, perment_add, email, phone_no, city, state, zip_code, gender, relationship_status, birth_date]);
            ...
            for (let i = 0; i < education_type.length; i++) {
                await connection.promise().query(sqledu, [id, education_type[i], nameboard[i], passyear[i], percentage[i]]);
            }

            // insert work-experience
            var sqlwork = `insert into work_experience (id,company_name,comdesg,from_date,to_date) values (?,?,?,?,?);`
            for (let i = 0; i < company_name.length; i++) {
                await connection.promise().query(sqlwork, [id, company_name[i], comdesg[i], from_date[i], to_date[i]]);
            }

            // insert refrances
            var sqlref = `insert into ref_contact (id,refname,refno,refrelation) values (?,?,?,?);`
            for (let i = 0; i < refname.length; i++) {
                await connection.promise().query(sqlref, [id, refname[i], refno[i], refrelation[i]]);
            }

            //  insert prefernces
            var sqlpref = `insert into preferances (id,location,department,notice_period,expacted_ctc,current_ctc) values (?,?,?,?,?,?);`
            await connection.promise().query(sqlpref, [id, location, department, noticeperiod, expactedctc, currentctc]);
            ...
            if (hindi) {
                ...
                await connection.promise().query(sqllang, [id, hindi, r_hindi, w_hindi, s_hindi]);
            }
            if (english) {
                if (r_eng == 'read') { r_eng = 1 } else { r_eng = 0 }
                if (w_eng == 'write') { w_eng = 1 } else { w_eng = 0 }
                if (s_eng == 'speak') { s_eng = 1 } else { s_eng = 0 }
                await connection.promise().query(sqllang, [id, english, r_eng, w_eng, s_eng]);
            }
            if (gujrati) {
                if (r_guj == 'read') { r_guj = 1 } else { r_guj = 0 }
                if (w_guj == 'write') { w_guj = 1 } else { w_guj = 0 }
                if (s_guj == 'speak') { s_guj = 1 } else { s_guj = 0 }
                await connection.promise().query(sqllang, [id, gujrati, r_guj, w_guj, s_guj]);
            }

            // insert technology
            var sqltech = `insert into technologies (id,tech_lang,tech_level) values (?,?,?);`;
            if (php) await connection.promise().query(sqltech, [id, php, tech_php]);
            if (mysql) await connection.promise().query(sqltech, [id, mysql, tech_mysql]);
            if (laravel) await connection.promise().query(sqltech, [id, laravel, tech_laravel]);
            if (oracle) await connection.promise().query(sqltech, [id, oracle, tech_oracle]);


            const msg = 'form was submitted successfully!';
            res.render('job-with-crud/form2', { msg });
        } catch (err) {
            console.log(err);
        }
    ...
    app.get('/jobwithcrud/edit/:id', async (req, res) => {
        const id = req.params.id;

        upbasic = `select * from basic_detail where id = ${id}`;
        var up = await connection.promise().query(upbasic);
        result = up[0][0];
        console.log(result);
        // res.send(result);

        upedu = `select * from education_master where id = ${id}`;
        var up1 = await connection.promise().query(upedu);
        result1 = up1[0];
        //console.log(result1);-

        upwork = `select * from work_experience where id = ${id}`;
        var up2 = await connection.promise().query(upwork);
        result2 = up2[0];
        //console.log(result2);

        upref = `select * from ref_contact where id = ${id}`;
        var up3 = await connection.promise().query(upref);
        result3 = up3[0];
        //console.log(result3);

        uppre = `select * from preferances where id = ${id}`;
        var up4 = await connection.promise().query(uppre);
        result4 = up4[0];
        //console.log(result4);

        uplang = `select * from language_known where id = ${id}`;
        var up5 = await connection.promise().query(uplang);
        result5 = up5[0];
        // console.log(result5)

        var php = []; var mysql = []; var laravel = []; var oracle = [];
        uptech = `select * from technologies where id = ${id}`;
        var up6 = await connection.promise().query(uptech);
        result6 = up6[0];
        // console.log(result6)
        ...
        res.render('job-with-crud/edit', { result6, php, mysql, laravel, oracle });
    });


    app.post('/jobwithcrud/edit/:id/update', async (req, res) => {
        const id = req.params.id;
        ...
        await connection.promise().query(updatedetail, [first_name, last_name, designation, current_add, perment_add, email, phone_no, city, state, zip_code, gender, relationship_status, birth_date]);
        ...
        for (let i = 0; i < education_type.length; i++) {
            await connection.promise().query(updateedu, [education_type[i], nameboard[i], passyear[i], percentage[i], edu_id[i]]);
        }
        ...
        for (let i = 0; i < company_name.length; i++) {
            await connection.promise().query(updatework, [company_name[i], comdesg[i], from_date[i], to_date[i], work_id[i]]);
        }

        // update ref-contact...
        let updateref = `update ref_contact set refname=?,refno=?,refrelation=? where id = ${id} and ref_id=?;`
        for (let i = 0; i < refname.length; i++) {
            await connection.promise().query(updateref, [refname[i], refno[i], refrelation[i], ref_id[i]]);
        }

        // update preferance..
        let updatepref = ` update preferances set location =?,department =?,notice_period =?,expacted_ctc =?,current_ctc=? where id= ${id};`
        await connection.promise().query(updatepref, [location, department, noticeperiod, expactedctc, currentctc]);
        ...
        if (hindi) {
            ...
            if (w_hindi == 'write') { w_hindi = 1 } else { w_hindi = 0 }
            if (s_hindi == 'speak') { s_hindi = 1 } else { s_hindi = 0 }
            await connection.promise().query(updatelang, [hindi, r_hindi, w_hindi, s_hindi, lang_id[0]]);
        }
        if (english) {
            if (r_eng == 'read') { r_eng = 1 } else { r_eng = 0 }
            if (w_eng == 'write') { w_eng = 1 } else { w_eng = 0 }
            if (s_eng == 'speak') { s_eng = 1 } else { s_eng = 0 }
            await connection.promise().query(updatelang, [english, r_eng, w_eng, s_eng, lang_id[1]]);
        }
        if (gujrati) {
            if (r_guj == 'read') { r_guj = 1 } else { r_guj = 0 }
            if (w_guj == 'write') { w_guj = 1 } else { w_guj = 0 }
            if (s_guj == 'speak') { s_guj = 1 } else { s_guj = 0 }
            await connection.promise().query(updatelang, [gujrati, r_guj, w_guj, s_guj, lang_id[2]]);
        }
        ...
        if (php) await connection.promise().query(updatetech, [php, tech_php, tech_id[0]]);
        if (mysql) await connection.promise().query(updatetech, [mysql, tech_mysql, tech_id[1]]);
        if (laravel) await connection.promise().query(updatetech, [laravel, tech_laravel, tech_id[2]]);
        if (oracle) await connection.promise().query(updatetech, [oracle, tech_oracle, tech_id[3]]);
